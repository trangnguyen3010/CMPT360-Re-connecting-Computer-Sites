...
public abstract class Graph280<V extends Vertex280, E extends Edge280<V>>
	...
	/**
	 * Fetch the edge from vertex v1 to vertex v2. If the edge does not exist, then an 
	 * ItemNotFound280Exception is thrown.
	 * @param v1 Source vertex for edge to retrieve
	 * @param v2 Destination vertex for edge to retrieve
	 * @return Edge object (v1,v2) in the graph.
	 * @throws ItemNotFound280Exception
	 */
	public abstract E getEdge(V v1, V v2) throws ItemNotFound280Exception;
	
	/**
	 * Fetch the edge from vertex with index srcIdx to vertex with index dstIdx.
	 * If the edge does not exist, then an ItemNotFound280Exception is thrown.
	 * @param srcIdx Index for source vertex of the edge to retrieve
	 * @param srcIdx Index for the destination vertex of the edge to retrieve
	 * @return Edge object (srcIdx, dstIdx) in the graph
	 * @throws ItemNotFound280Exception
	 */
	public E getEdge(int srcIdx, int dstIdx) throws ItemNotFound280Exception
	{
		return this.getEdge(this.vertex(srcIdx), this.vertex(dstIdx));
	}
